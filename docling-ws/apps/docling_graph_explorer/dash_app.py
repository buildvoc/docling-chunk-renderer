...
def _demo_elements() -> List[Dict[str, Any]]:
    # From dash-cytoscape usage-elements-extra demo (simplified)
    return [
...
def _demo_stylesheet() -> List[Dict[str, Any]]:
    return [
        {"selector": "node", "style": {"label": "data(label)", "text-halign": "center", "text-valign": "center"}},
...
def create_dash_app(base_path: str = "/dash") -> Dash:
    ...
    def serve_cytoscape_asset(asset: str):
        # Serve dash_cytoscape assets, forgiving versioned filenames.
        available = [item.get("relative_package_path") for item in (cyto._js_dist or []) if item.get("relative_package_path")]
        fname = asset.split("/")[-1]
        match = None
        for rel in available:
            ...
            if fname == rel_name:
                match = rel
                break
            base = rel_name.split(".min.js")[0]
            if fname.startswith(base) and fname.endswith(".js"):
                match = rel
                break
        if not match:
            logger.info("dash asset request miss: %s (available=%s)", asset, available)
            return ("Not Found", 404)
        import pkgutil

        data = pkgutil.get_data("dash_cytoscape", match)
        if data is None:
            return ("Not Found", 404)
    ...
    def refresh(_n):
        ...
        mode = "Demo elements (fallback)"
        if elements:
            mode = "Shared state elements"
        else:
            elements = _demo_elements()
        ...
        indicator = (
            f"{meta.get('showing', '')} | "
            f"nodes={meta.get('nodes', 0)} edges={meta.get('edges', 0)}"
        )
